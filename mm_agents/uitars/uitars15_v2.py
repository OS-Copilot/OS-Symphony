...
def parse_action_to_structure_output(text, factor, origin_resized_height, origin_resized_width, model_type="qwen25vl",
    ...
    if model_type == "qwen25vl":
        smart_resize_height, smart_resize_width = smart_resize(origin_resized_height, origin_resized_width,
                                                               factor=IMAGE_FACTOR, min_pixels=min_pixels,
...
def parsing_response_to_pyautogui_code(responses, image_height: int, image_width: int, input_swap: bool = True,
                                       platform: str = "Ubuntu") -> str:
    '''
    ...
    def predict(self, task_instruction: str, obs: dict):
        ...
        for _ in range(self.critic_times):
            ...
            while True:
                ...
                try:
                    parsed_dict = parse_action_to_structure_output(prediction, self.action_parse_res_factor,
                                                                   origin_resized_height, origin_resized_width,
                                                                   self.model_type)
                    parsed_pyautogui_code = parsing_response_to_pyautogui_code(parsed_dict, origin_resized_height,
            ...
            critic_flag, critic_text = self.critic_agent.critic(task=task_instruction, screenshot=obs["screenshot"],
        ...
        try:
            parsed_dict = parse_action_to_structure_output(prediction, self.action_parse_res_factor,
                                                           origin_resized_height, origin_resized_width, self.model_type)
            parsed_pyautogui_code = parsing_response_to_pyautogui_code(parsed_dict, origin_resized_height,
                                                                       origin_resized_width, platform=self.platform)
