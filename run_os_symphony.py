...
def run_env_tasks(
    ...
    try:
        ...
        # Search Env Init
        if args.searcher_provider_name == "docker":
        ...
        elif args.searcher_provider_name == "aws":
            from desktop_env.osworld.providers.aws.manager import IMAGE_ID_MAP
            ami_id = IMAGE_ID_MAP[region].get(screen_size, IMAGE_ID_MAP[region][(1920, 1080)])
            search_env = OSWorldDesktopEnv(
                path_to_vm=args.path_to_vm,
            ...
            raise Exception("Don't support other search providers!")
...
def config() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--searcher_provider_name",
        type=str,
        default="docker",
        help="Type of search provider_name, support docker and aws, default is docker",
    )
    parser.add_argument(
        "--searcher_budget",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1920,
        help="Search environment's width",
    )
    parser.add_argument(
        "--searcher_screen_height",
        type=int,
        default=1080,
        help="Search environment's height",
    )
    parser.add_argument(
        "--searcher_path_to_vm",
        type=str,
        default=None,
        help="Searcher Env VM's path (=OSWorld's VM Path)",
    )
